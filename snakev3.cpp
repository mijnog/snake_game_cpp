...
const int mapHeight = 30; // Reduced height to accommodate borders
...
void initMap() {
    ...
    for (int x = 0; x < mapWidth; ++x) {
        map[x] = -3; // Top border
        map[(mapHeight - 1) * mapWidth + x] = -3; // Bottom border
    }
    for (int y = 0; y < mapHeight; ++y) {
        map[y * mapWidth] = -3; // Left border
        map[y * mapWidth + (mapWidth - 1)] = -3; // Right border
    }
}
